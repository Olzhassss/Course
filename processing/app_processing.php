...
try {
	...
	if ($_POST['application_type']=='teacher') // If application is a teacher form
	{
		...
		$_POST['summary'] = is_valid(filtrateString( $_POST['summary']), '.{0,2500}');
		if (strlen($_POST['preferences'])>2500)
		...
		// Throw an expection if inquery yields any result
		if ($stmt->fetchColumn())
		...
		$sql = 'INSERT INTO appletree_personnel.app_teachers
			(name, surname, app_date, birth_year phone_number, email, ed_lvl, exp, gender, summary, opt_radio1, opt_radio2, opt_radio3)
			VALUES (:name, :surname, :app_date, :birth_year, :phone_number, :email, :ed_lvl, :exp, :gender, :summary, :opt_radio1, :opt_radio2, :opt_radio3)';
	...
	elseif($_POST['application_type']=='student') // If application is a student form
	{
		...
		$sql = 'INSERT INTO appletree_personnel.app_students
			(name, surname, gender, birth_year, phone_number, email, group_ls, ed_lvl, app_date, opt_checkbox1, opt_checkbox2, preferences)
			VALUES (:name, :surname, :gender, :birth_year, :phone_number, :email, :group_ls, :ed_lvl, :app_date, :opt_checkbox1, :opt_checkbox2, :preferences)';
		$stmt = $pdo->prepare($sql);
		$stmt->execute([
			':name' => $_POST['name'],
			':surname' => $_POST['surname'],
